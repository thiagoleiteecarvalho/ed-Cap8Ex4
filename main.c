/*
 * main.c
 * Resposta do exercício 4 do capítulo 8.
 * Author: Thiago Leite
 */
...
int main(void) {
	...
	do {
		...
		case 2:
			...
			scanf(" %d", &valor);
			set_raiz(insert(get_raiz(), valor));
			break;
		...
		case 4:
			...
			No *no_delete = NULL;
			no_delete = delete(get_raiz(), valor);
			if (no_delete != NULL) {
			...
			break;
		case 5:
			exibir_arvore(get_raiz(), 1);
			printf("\n\n\n");
			exibir_em_ordem(get_raiz());
			break;
		case 6:
			printf("A altura da árvore é %d.", altura_arvore(get_raiz()));
			break;
		default:
